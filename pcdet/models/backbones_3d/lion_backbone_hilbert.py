...
from ..model_utils.voxel_mamba_utils import get_z_axe_hilbert_index_3d_mamba_lite
...
class FlattenedHilbertMapping(nn.Module):
    def __init__(
        self,
        # window_shape,
        # group_size,
...
class LION3DBackboneHilbert(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        self.template_path = model_cfg.TEMPLATE_PATH
        self.template_rank = model_cfg.TEMPLATE_RANK
        self.template = {}
        for rank in self.template_rank:
            self.load_template(rank)
    ...
    def load_template(self, order):
        ...
        if isinstance(template, dict):
            ...
            template_dict['curve'] = template['data'].reshape(-1)
            template_dict['size'] = template['size']
        else:
